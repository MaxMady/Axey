  ...
  if (price) {
    if (typeof price === 'number') {
      if (robux !== price) {
        ...
        return `Price requirement not met.  Requested price: ${price} Actual price: ${robux}`
      ...
      if (high) {
        if (robux > high) {
          return `Price requirement not met.  Requested price: <=${high} Actual price: ${robux}`
  ...
  return http(httpOpt)
    .then(function (json) {
      ...
      if (!err) {
        ...
        return `Price has been altered!` + err;
        throw new Error(err)
      }
    })
}
